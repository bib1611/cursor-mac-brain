---
type: agent-bus-task
id: 20260619-202054-claude-agent-canvas-mission
created: 2026-06-19 20:20:54
source: telegram
...
# Agent Canvas mission:

## Request

Agent Canvas mission:

Build brief from Adam/Codex for CNVS on Mac mini.

Goal: build the smallest useful landing-page variant inspired by Dino Anthony / Art of Purpose offer structure.

Context loaded by Codex:
- Dino / Art of Purpose pages are mostly narrow Gumroad-style offers: `Create, Publish, Profit / 10X Twitter`, `101 Ways to Write a Tweet`, `Content Academy`, `First 1,000 Email Subs`.
- Useful pattern: outcome + platform/problem + timeframe. Example shape: `20-Day Sprint`, `10X Your X`, `101 Ways...`.
- Our PBM2 page is stronger as a full direct-response page, but heavier. Current source: `/Users/adamjohnsson/code/deadhidden/src/app/store/plain-bible-2.0/page.tsx`.
- Existing checkout rails/components must stay intact. Do not create new checkout/product/payment logic.

Build request:
1. Create a minimal PBM2 above-the-fold variant or patch proposal named around `14-Day Plain Bible Reset`.
2. Reuse the existing `/store/plain-bible-2.0` page, `Pbm2CheckoutForm`, pricing constants, product images, and current checkout behavior.
3. First viewport should make the offer read like a clear sprint:
   - eyebrow: `14-Day Plain Bible Reset`
   - headline direction: read your King James plain again without seminary fog or AI correction
   - subcopy: manual + workbook + Scripture-safe prompts, instant PDF system
   - CTA: complete system path, $37
   - preserve upgrade-mode language for existing buyers
4. Keep the long-form sections underneath unless there is a clear reason to trim.
5. Deliver either a local patch or exact changed-file plan with line references.
6. Verify with the smallest runnable check available. No deploy, no publish, no X/Substack/email action, no Stripe/customer mutation.

Claude lane: critique the copy/structure and flag what would make the first viewport sharper without making the page generic.
Codex lane: implement or produce the smallest safe patch against the current repo.

Use the Current Team Context Packet attached by dh-ops as shared memory. Mark stale claims as VERIFY. Leave changed paths, proof, and receipt paths in your result.

Hard boundary: do not publish, send external messages, spend, refund, deploy, delete important files, change credentials, or change account settings without Adam's explicit same-thread approval of that exact action.

Claude lane:
- Work as the specialist second brain for writing, code review, or system design.
- Produce concise critique, missing assumptions, and a concrete draft/patch plan.
- Do not claim local file truth unless a Codex-provided packet or readable artifact proves it.

## Dakota Robertson addendum

Codex checked the local Dakota/Growth Ghost archive and current public funnel sources after Adam said to look at Dakota's stuff:
- local archive: `/Users/adamjohnsson/iCloud Drive (Archive)/Documents/Obsidian Vault/10 Sources/Dakota Robertson - Growth Ghost (study)/`
- local reference: `/Users/adamjohnsson/cowork/Claude-Cowork/memory/auto-memory-2026-04-21/reference_library_dakota-robertson.md`
- public Beehiiv home/archive: `https://dakota.beehiiv.com/`
- public sales page: `https://growthwriting.com/growth-writing`
- safety note: `/Users/adamjohnsson/Downloads/bm-honest-algorithm-acquisition-email.md`

Use these mechanics, not Dakota's tone:
- Reposition commodity to outcome: Dakota moves from "writing" to "growth writing"; PBM2 should move from "Bible manual" to `14-Day Plain Bible Reset` or "KJV reading system."
- Use a transformation hook: reader now -> reader after the reset.
- Add one short story/bridge beat if it sharpens the hero: confusion, fog, or over-explained Bible reading -> plain KJV reading again.
- Keep the first screen concrete: promise, specific deliverable stack, price, instant access, and proof/guarantee if already supported by existing page assets.
- Use low-friction CTA language. Do not make the page generic or creator-guru.

Hard boundary: do not import Dakota's profanity, crude shock tone, or aggressive guru voice into Dead Hidden/Biblical Man copy. Do not name Dakota, Pascio, or acquisition claims in public copy unless a public sale is verified.


## Current Team Context Packet

Generated automatically by `dh-ops` when this task was created.
...
`/Users/adamjohnsson/Documents/Obsidian Vault/70 Systems/Active Ops Capsule/TEAM_CONTEXT.md`

Remote-agent note:

If the target agent cannot read local Mac paths, paste this section into that agent's thread.

# Dead Hidden Agent Team Context

Generated: 2026-06-19 20:20 CDT
...
- Read local files, Obsidian notes, repos, dashboards, and public/live pages as needed.
- Draft copy, support replies, briefs, and implementation plans.
- Write receipts back to Obsidian.

## Do Not Do Without Adam Confirming In The Same Thread

- Send emails or external messages.
- Publish, delete, or edit live Substack/X/Threads content.
- Charge, refund, cancel, dispute, or modify customer/payment accounts.
- Deploy production changes.
- Delete files outside generated scratch/output.
- Change API keys, DNS, cron, automations, or payment methods.

## Proof Required

- State which tools/files/accounts were checked.
- Include paths, URLs, IDs, or timestamps where safe.
- Mark stale or missing claims as `VERIFY`.

## Result Slot

Write result to:

`/Users/adamjohnsson/Documents/Obsidian Vault/70 Systems/Agent Bus/results/20260619-202054-claude-agent-canvas-mission.md`
